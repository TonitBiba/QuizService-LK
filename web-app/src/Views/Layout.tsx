...
import { AppBar, Badge, Box, Container, FormControlLabel, IconButton, Toolbar, Typography } from "@mui/material";
...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  const ChangeTheme = () => {
    dispatch(theme.actions.SetThemeMode(themeReducer.mode === "light" ? "dark" : "light"));
  };
  ...
  return (
    <>
      <AppBar position="relative">
        <Toolbar>
          <Stack onClick={() => navigate("/")} direction={"row"} sx={{ cursor: "pointer" }}>
            <SchoolIcon sx={{ mr: 2 }} />
            <Typography variant="h6" color="inherit" noWrap>
              {intl.formatMessage({ id: "OnlineTestingApp" })}
            </Typography>
          </Stack>
          <Box sx={{ flexGrow: 1 }} />
          <Box sx={{ display: { xs: "none", md: "flex" } }}>
            <FormControlLabel control={<MaterialUISwitch sx={{ m: 1 }} checked={themeReducer.mode === "dark"} onClick={ChangeTheme} />} label="" />
            <Language />
          </Box>
        </Toolbar>
      </AppBar>
      <Box mt={4}>
        {themeReducer.alert != null ? (
          <Box mt={2}>
            <AlertShared error={themeReducer.alert as ErrorVM} setError={() => {}} />
          </Box>
        ) : (
          <></>
        )}
